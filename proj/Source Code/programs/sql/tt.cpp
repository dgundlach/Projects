...
-- Find all unreconciled fees and process them if possible.
    ...
    for t_rec in select *
                  ...
                  where acctno = tacctno and r_seq = -1
                        and type >= FEE_TYPE_MIN
                        and type <= FEE_TYPE_MAX
               order by t_seq loop
...
-- Mark the setup fee as paid.
            ...
            if t_rec.type = TRANS_SETUP_FEE then
                user.setup_paid := ''Y'';

-- Add 1 to the expire interval.

            elsif t_rec.type = TRANS_MONTHLY_FEE then
    ...
    if res > 0 then
        ...
        insert into transactions 
             ...
             values (tacctno, user.t_seq, current_user, 
                     TRANS_PAYMENT_FROM_RESERVE, res, transno);
    end if;
    if cred > 0 then
        user.t_seq := user.t_seq + 1;
        insert into transactions
                    (acctno, t_seq, entered_by, type, amount, r_seq)
             values (tacctno, user.t_seq, current_user, 
                     TRANS_PAYMENT_FROM_CREDIT, cred, transno);
    end if;
        ...
        if total_pd <= user.total_paid then
            ...
            insert into transactions 
                        (acctno, t_seq, entered_by, type, amount,
                         r_seq, comment) 
                 values (tacctno, user.t_seq, current_user, 
                         TRANS_REFERRAL_ENABLED, 0, transno, 
...
-- If the customer has any pending referral credits, apply one of them.
    ...
    if user.referral_credits > 0 then
        ntype := MOVE_REFERRAL_MONTH_TO_CREDITS;
        namount := tamount;
...
-- If the customer has prepaid, apply the payment.
        ...
        ntype := MOVE_PREPAID_MONTH_TO_RESERVE;
...
-- If the customer has any credit months, apply one.
        ...
        ntype := MOVE_FREE_MONTH_TO_CREDITS;
        namount := tamount;
        user.credit := user.credit + namount;
...
-- Check if the account is in arrears, and suspend it if that is the case.
        ...
        if user.billing_interval > 0 and amt > user.monthly_fee then
            ...
            insert into transactions
                       (acctno, t_seq, entered_by, type, amount, r_seq) 
                values (tacctno, user.t_seq, current_user, 
                        TRANS_AUTO_SUSPENDED_ACCOUNT, 0, ntransno);
            user.suspended := ''A'';
        end if;
    end if;
...
create or replace function 
          ...
          billing, integer) returns integer as SQUOTE
declare
    tacctno   alias for $1;
    ttype     alias for $2;
    tamount   alias for $3;
...
-- and generate a transaction for them.
    ...
    free := months / TRANS_PREPAY_FOR_FREE_MONTH;
    if free > 0 then
        user.t_seq := user.t_seq + 1;
        insert into transactions
                   (acctno, t_seq, entered_by, type, amount, r_seq, months)
...
-- bring the outstanding transactions in line with the new price per month.
    ...
    if diff > 0 then
        ...
        insert into transactions
                   (acctno, t_seq, entered_by, type, amount, r_seq) 
            values (tacctno, user.t_seq, current_user, 
                    TRANS_PREPAYMENT_ADJUSTMENT, diff, transno);
...
-- Be sure to put what is happening to the reserve into the audit trail.
    ...
    if overpay != 0 then
        ...
        insert into transactions
                   (acctno, t_seq, entered_by, type, amount, r_seq) 
            values (tacctno, user.t_seq, current_user, 
                    MOVE_PAYMENT_TO_RESERVE, overpay, transno);
    end if;
...
-- block.
    ...
    insert into transactions
               (acctno, t_seq, entered_by, type, amount, r_seq, months)
        values (tacctno, user.t_seq, current_user,
                MOVE_PAYMENT_TO_PREPAID, ppm * months, transno,
                months);
...
-- for the transactions to the reserve.
    ...
    if cnt > 0 then
        ...
        insert into transactions
                   (acctno, t_seq, entered_by, type, amount, r_seq) 
            values (tacctno, user.t_seq, current_user, 
                    MOVE_PREPAID_MONTH_TO_RESERVE, ppm * cnt, transno);
    end if;
...
begin
    ...
    select into t_rec * 
          ...
          where acctno = tacctno and amount = tamount
                                      and type >= FUNDS_IN_MIN
                                      and type <= FUNDS_IN_MAX
                                      and revoked = ''N''
       order by t_seq desc
          limit 1;
    parent := t_rec.t_seq;
    ...
    for t_rec in select *
        ...
        if t_rec.type < FEE_TYPE_MIN then
            update transactions set revoked = ''Y'',
                                    r_seq = t_seq
                              where acctno = tacctno
                                and t_seq = t_rec.t_seq;
        elsif t_rec.type >= FEE_TYPE_MIN
                and t_rec.type <= FEE_TYPE_MAX then
            update transactions set r_seq = -1
                              where acctno = tacctno
                                and t_seq = t_rec.t_seq;
        end if;

        if t_rec.type = MOVE_PAYMENT_TO_RESERVE then
            res := res - t_rec.amount;

        elsif t_rec.type = TRANS_PAYMENT_FROM_RESERVE then
            res := res + t_rec.amount;

        elsif t_rec.type = MOVE_PAYMENT_TO_PREPAID then
            pm := pm - t_rec.months;
            usepm := true;

        elsif t_rec.type = MOVE_PREPAID_MONTH_TO_RESERVE then
            pm := pm + t_rec.months;
            usepm := true;

        elsif t_rec.type = TRANS_CREDIT_EXTERNAL then
            cred := cred - t_rec.amount;

        elsif t_rec.type = TRANS_PREPAYMENT_ADJUSTMENT then
            cred := cred - t_rec.amount;

        elsif t_rec.type = TRANS_PAYMENT_FROM_CREDIT then
            cred := cred + t_rec.amount;

        elsif t_rec.type = TRANS_GRANT_FREE_MONTHS then
            cm := cm - t_rec.months;
            usecm := true;

        elsif t_rec.type = MOVE_FREE_MONTH_TO_CREDITS then
            cm := cm + t_rec.months;
            usecm := true;

        elsif t_rec.type >= TRANS_CASH_FUNDS
                and t_rec.type < FEE_TYPE_MIN then
        ...
        elsif t_rec.type = TRANS_SETUP_FEE then
            bal := bal + t_rec.amount;
            user.setup_fee_paid := ''N'';

        elsif t_rec.type = TRANS_PENALTY_FEE
                or t_rec.type = TRANS_MISC_FEE then
            bal := bal + t_rec.amount;
    ...
    user.credit := user.credit + cred;
    user.reserve := user.reserve + res;
    if parent_t >= TRANS_SETUP_PREPAID_CASH
            and parent_t <= TRANS_SETUP_PREPAID_CHECK then
    ...
    return reconcile_amounts(tacctno, TRANS_REVOKE_FUNDS, user, transno);
end;
SQUOTE language 'plpgsql';


create or replace function preprocess_trans() returns trigger as SQUOTE
declare
    rename   new to trans;
    transno  int;
...
-- Only fetch the user record if we absolutely have to.
    ...
    if trans.type >= MIN_TAKE_ACTION and trans.type < TRANS_ADDED_ACCOUNT then
        select into user * from billing where acctno = trans.acctno;
    end if;
    if trans.type = TRANS_MONTHLY_FEE then
...
-- Setup fee and other fees.
    ...
    elsif trans.type in (TRANS_SETUP_FEE,
                         TRANS_PENALTY_FEE,
                         TRANS_MISC_FEE)            then
...
-- Payment from cash, check, credit card, etc.
    ...
    elsif trans.type in (TRANS_CASH_FUNDS,
                         TRANS_CREDIT_CARD_FUNDS,
                         TRANS_DEBIT_CARD_FUNDS,
                         TRANS_CHECK_FUNDS,
                         TRANS_AUTO_CHECK_FUNDS)    then
        user.t_seq := user.t_seq + 1;
        insert into transactions
                   (acctno, t_seq, entered_by, type, amount, r_seq) 
...
-- Revoke funds entered by mistake or for insufficient funds.
    ...
    elsif trans.type = TRANS_REVOKE_FUNDS           then
...
-- in the middle of the month to prorate his first month.
    ...
    elsif trans.type = TRANS_CREDIT_EXTERNAL_IN     then
        user.credit := user.credit + trans.amount;
        trans.type := TRANS_CREDIT_EXTERNAL;
        i := reconcile_amounts(trans.acctno, trans.type, user, transno);

-- Add the account or "unremove" it.

    elsif trans.type = TRANS_ADDED_ACCOUNT          then
        update billing set t_seq = t_seq + 1, 
                           removed = ''N''
                     where acctno = trans.acctno;
...
-- hide the account from queries in the user level programs.
    ...
    elsif trans.type = TRANS_REMOVED_ACCOUNT        then
...
-- Lock the account.
    ...
    elsif trans.type = TRANS_LOCKED_ACCOUNT         then
        update billing set t_seq = t_seq + 1,
                           locked = ''Y''
                     where acctno = trans.acctno;
...
-- Unlock the account.
    ...
    elsif trans.type = TRANS_UNLOCKED_ACCOUNT       then
        update billing set t_seq = t_seq + 1,
                           locked = ''N''
                     where acctno = trans.acctno;
...
-- type of suspension used by these routines.
    ...
    elsif mew.type = TRANS_SUSPENDED_ACCOUNT      then
        update billing set t_seq = t_seq + 1,
                           suspended = ''Y''
...
-- Unsuspend the account.  This works for both types of suspension.
    ...
    elsif trans.type = TRANS_UNSUSPENDED_ACCOUNT    then
        update billing set t_seq = t_seq + 1,
                           suspended = ''N''
                     where acctno = trans.acctno;
...
-- Set up a block of prepaid months.
    ...
    elsif trans.type in (TRANS_SETUP_PREPAID_CASH,
                         TRANS_SETUP_PREPAID_CREDIT_CARD,
                         TRANS_SETUP_PREPAID_DEBIT_CARD,
                         TRANS_SETUP_PREPAID_CHECK) then
